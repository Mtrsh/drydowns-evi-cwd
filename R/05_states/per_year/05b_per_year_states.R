# =============================================================================
# YEARLY STATES, STEP B of C : PEAK AND LOW STATES
# Original file name: Percentiles_per_year_worker_v2.R
# Methods 2.6
# -----------------------------------------------------------------------------
# For each grid cell and year of one group, we take the weekly (CWD, EVI) values of the largest
# drydown of that year only, and compute two states:
#
...
# Output : per_year_chunked/<zone>/results/<zone>_chunk_<id>_results.Rdata
#            (object percentiles_summary, one row per grid cell and year:
#            Latitude, Longitude, n_observations, EVI_E95, CWD_atE95,
#            EVI_E05, CWD_atE05, p05_position, EVI_max, CWD_max, EVI_range,
#            climate_zone,
#            plus year_id and Year, used to group the data)
#
# Run    : one job per zone and group:
#            Rscript 05b_per_year_states.R tropical 3
#          If one grid cell and year fails, it is skipped and the job goes on.
# =============================================================================
...
write_log(paste("STEP 2b PER-YEAR v2 — chunk", CHUNK_ID, "zone:", zone))
...
if (nrow(chunk_gridcells) == 0) stop("No grid cells")
...
write_log("Starting per-year processing...")
n_total  <- uniqueN(drydown_data$year_id)
counter  <- 0L
last_log <- Sys.time()
...
results <- drydown_data[, {
  ...
  sd_with_keys[, `:=`(lat = .BY$lat, lon = .BY$lon, year = .BY$year)]
  list(result = list(tryCatch(process_year_percentiles(sd_with_keys), error = function(e) NULL)))
}, by = .(lat, lon, year)]

results_list <- results$result[!sapply(results$result, is.null)]
write_log(paste("Processed pixel-years:", length(results_list)))
