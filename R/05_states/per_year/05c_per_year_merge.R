# =============================================================================
# YEARLY STATES, STEP C of C : MERGE THE GROUPS
# Original file name: Percentiles_per_year_merge_v2.R
# Methods 2.6
# -----------------------------------------------------------------------------
# Merges the results of step B into one table per zone and one table for all
# zones. If a group was run twice, the duplicates are removed (by year_id).
# Zone names are changed to the form used in the figures ("arid_hot" becomes
# "Arid Hot").
#
# Input  : per_year_chunked/<zone>/results/<zone>_chunk_*_results.Rdata
# Output : <zone>/<zone>_per_year_percentiles_summary_v2.Rdata
#          combined_per_year_percentiles_summary_v2.Rdata (object all_summaries).
#          This combined file is used for the trends (06_trends_theil_sen.R,
#          Methods 2.8) and the figures for single years.
#
# Run    : Rscript 05c_per_year_merge.R, when all step B jobs are finished.
# =============================================================================
...
chunk_path        <- "/Net/Groups/BGI/scratch/mterristi/PhD/CWD_0_1/New_Climate_Zones_percentiles/Drydowns/Drydowns_percentiles_per_year_v2/per_year_chunked"
final_output_path <- "/Net/Groups/BGI/scratch/mterristi/PhD/CWD_0_1/New_Climate_Zones_percentiles/Drydowns/Drydowns_percentiles_per_year_v2/"
...
for (zone in zones) {
  ...
  percentiles_summary <- unique(percentiles_summary, by = "year_id")
  ...
  cat("  Total pixel-years:", nrow(percentiles_summary), "\n")
  
  zone_output_dir <- file.path(final_output_path, zone)
  dir.create(zone_output_dir, showWarnings = FALSE, recursive = TRUE)
  output_file <- file.path(zone_output_dir, paste0(zone, "_per_year_percentiles_summary_v2.Rdata"))
...
cat("Combined pixel-years:", nrow(all_summaries), "\n")

combined_file <- file.path(final_output_path, "combined_per_year_percentiles_summary_v2.Rdata")
save(all_summaries, file = combined_file)
cat("Saved combined:", combined_file, "\n")

cat("\n=== STEP 2d PER-YEAR MERGE v2 DONE ===\n")
